...
class APIService:
    ...
    def _safe_query_companies(self, sector: str) -> List[dict]:
        try:
            companies = self.db.exec(select(Company).where(Company.sector == sector).limit(20)).all()
            return [{"id": c.id, "name": c.name, "county": c.county, "sector": c.sector} for c in companies] if companies else []
        except:
            return []

    def _safe_query_metrics(self, sector: str, county: str = None) -> List[dict]:
        try:
            query = select(MarketMetric).where(MarketMetric.sector == sector)
            if county:
                query = query.where(MarketMetric.county == county)
            metrics = self.db.exec(query.limit(50)).all()
    ...
    async def get_competitive(self, sector: str, county: str = None) -> Dict[str, Any]:
        try:
            ...
            if not data:
                data = self._safe_query_companies(sector)
        except Exception:
            data = self._safe_query_companies(sector)

        if county and isinstance(data, list):
            data = [c for c in data if isinstance(c, dict) and c.get("county") == county]
    ...
    async def get_price_oracle(self, sector: str, county: str = None) -> Dict[str, Any]:
        try:
            ...
            if not data:
                data = self._safe_query_metrics(sector, county)
        except Exception:
            data = self._safe_query_metrics(sector, county)
    ...
    async def get_policy(self, sector: str = None) -> Dict[str, Any]:
        try:
            query = select(KnowledgeChunk).where(KnowledgeChunk.category == "policy")
            if sector:
                query = query.where(KnowledgeChunk.sector == sector)
            policies = self.db.exec(query.limit(20)).all()
    ...
    async def get_funding(self, sector: str) -> Dict[str, Any]:
        try:
            ...
            print(f"Funding tracker failed: {e}")
            data = self._safe_query_companies(sector)
    ...
    async def get_risk(self, business: str, county: str) -> Dict[str, Any]:
        try:
            ...
            data = {"business": business, "county": county, "risk_level": "Unknown - no data yet", "message": "Run seed_master to populate risk data"}
    ...
    async def get_export(self, sector: str) -> Dict[str, Any]:
        try:
            exports = self.db.exec(select(ExportOpportunity).where(ExportOpportunity.sector == sector).limit(20)).all()
    ...
    async def get_county(self, county: str) -> Dict[str, Any]:
        try:
            ...
            if not data:
                metrics = self.db.exec(select(MarketMetric).where(MarketMetric.county == county).limit(20)).all()

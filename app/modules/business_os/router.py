...
class BusinessCreate(BaseModel): name: str; sector: str; county: str
class ProductCreate(BaseModel): business_id: int; name: str; sku: str; selling_price: float; buying_price: float = 0.0; stock_qty: int = 0
class InvoiceCreate(BaseModel): business_id: int; customer_name: str; customer_phone: str; total_amount: float
class EmployeeCreate(BaseModel): business_id: int; full_name: str; phone: str; role: str; salary_kes: float
class PayInvoice(BaseModel): invoice_id: int; mpesa_receipt: str
...
def get_business_details(request: Request, business_id: int, session: Session = Depends(get_session)):
    ...
    if not business or business.owner_id != user_id: raise HTTPException(status_code=404, detail="Business not found")
    return business
...
def add_new_product(request: Request, req: ProductCreate, session: Session = Depends(get_session)):
    user_id = request.state.user.id
    consume_credits(session, user_id, "api_credits", 1)
    return add_product(session, req, user_id)
...
def create_new_invoice(request: Request, req: InvoiceCreate, session: Session = Depends(get_session)):
    ...
    return create_invoice(session, req, user_id)

@router.post("/accounting/invoice/pay")
@require_module(module_number=8)
def pay_invoice(request: Request, req: PayInvoice, session: Session = Depends(get_session)):
    user_id = request.state.user.id
    return mark_invoice_paid(session, req.invoice_id, req.mpesa_receipt, user_id)
...
def add_new_employee(request: Request, req: EmployeeCreate, session: Session = Depends(get_session)):
    user_id = request.state.user.id
    consume_credits(session, user_id, "api_credits", 1)
    return add_employee(session, req, user_id)

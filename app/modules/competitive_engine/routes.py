...
async def company_db(sector: str, company_name: str = None, db: Session = Depends(get_db)):
...
async def funding(sector: str, investor: str = None, date_range: str = "90d", db: Session = Depends(get_db)):
    service = CompetitiveEngineService(db)
    return await service.funding_tracker(sector, investor, date_range)

@router.get("/api/competitive/traffic")
async def traffic(competitor1: str, competitor2: str, date_range: str = "30d", db: Session = Depends(get_db)):
    service = CompetitiveEngineService(db)
    return await service.digital_traffic_analyzer(competitor1, competitor2, date_range)

@router.get("/api/competitive/monitor")
async def monitor(competitor: str, signal_type: str, db: Session = Depends(get_db)):
    service = CompetitiveEngineService(db)

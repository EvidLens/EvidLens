...
from app.core.models import KenyaLensSubscription  # This is correct
...
def require_active_subscription(current_user: AuthUser = Depends(get_current_user), db: Session = Depends(get_db)):
    sub = db.exec(select(KenyaLensSubscription).where(KenyaLensSubscription.user_id == current_user.id)).first()  # FIXED HERE
    if not sub:

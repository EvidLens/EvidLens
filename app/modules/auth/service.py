...
def create_user(db: Session, req, token: str):
    ...
    db_user = AuthUser(email=req.email, phone=req.phone, hashed_password=hashed_pw, full_name=req.full_name, sector=req.sector, county=req.county, verification_token=token, role=UserRole.USER, is_active=True, email_verified=False)
    db.add(db_user)
    db.commit()
    db.refresh(db_user)
    verify_link = f"{APP_URL}/auth/verify?token={token}"
    send_email(req.email, "Verify your EvidLens Account", f"<h2>Welcome</h2><p><a href='{verify_link}'>Verify Email</a></p>")
    return db_user
...
def login_user(db: Session, email: str, password: str):
    ...
    if not user.email_verified:
        return {"error": "Email not verified - check inbox"}
...
def request_password_reset(db: Session, email: str):
    ...
    send_email(email, "Reset your EvidLens Password", f"<p>Reset: <a href='{reset_link}'>Reset Password</a></p>")

...
TIER_PRICES = {
    ...
    SubscriptionTier.SME_STARTER: 500,
...
def stk_push(request: STKPushRequest, db: Session = Depends(get_db)):
    result = initiate_stk_push(
        ...
        account_reference=str(request.reference_id) if request.reference_id else request.description,
        user_id=0
    )
...
def buy_report(report_id: int, phone_number: str, bundle: str = "single", user_id: int = 0, db: Session = Depends(get_db)):
    amount = REPORT_BUNDLES.get(bundle, REPORT_BUNDLES["single"])
    stk = initiate_stk_push(
        db=db,
    ...
    return {"success": True, "checkout_request_id": stk.get("CheckoutRequestID"), "amount": amount, "bundle": bundle}

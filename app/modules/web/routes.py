...
def dashboard():
   ...
   .card{background:white;border:1px solid #e2e8f0;border-radius:16px;padding:24px;color:#0f172a}
   ...
   .live{color:#f97316;font-size:12px;font-weight:600}.scroll{max-height:250px;overflow:auto;margin-top:10px}
   ...
   .footer{background:#1e3a8a;color:white;padding:40px;margin-top:50px}.chat-btn{position:fixed;bottom:20px;right:20px;width:56px;height:56px;border-radius:50%;background:#0ea5e9;color:white;border:none;font-size:24px;cursor:pointer}
    ...
    async function loadCards(){let html='';for(let m of modules){let res=await fetch(`/api/${m.key}?page=1&page_size=50`).then(r=>r.json());html+=`<div class="card"><div class="icon">${m.icon}</div><div class="title">${m.title}</div><div class="subtitle">${res.count} Records</div><div class="scroll"><table>`;if(res.count>0){res.data.slice(0,10).forEach(r=>{html+=`<tr><td>${Object.values(r).slice(0,4).join('</td><td>')}</td></tr>`})}else{html+=`<tr><td>0 records. Ready for data.</td></tr>`}html+=`</table></div><div class="live">LIVE</div></div>`}document.getElementById('grid').innerHTML=html}
    loadStats();loadCards();
    </script></body></html>"""
